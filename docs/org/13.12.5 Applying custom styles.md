...
The ODT export backend comes with many OpenDocument styles (see [Working with OpenDocument style files](/docs/org/Advanced-topics-in-ODT-export)). To expand or further customize these built-in style sheets, either edit the style sheets directly or generate them using an application such as LibreOffice. The example here shows creating a style using LibreOffice.
...
3.  Customize the variable `org-odt-styles-file` and point it to the newly created file. For additional configuration options, see [Overriding factory styles](/docs/org/Advanced-topics-in-ODT-export).
