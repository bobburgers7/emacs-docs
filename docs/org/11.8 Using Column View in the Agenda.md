...
2.  If any of the columns has a summary type defined (see [Column attributes](/docs/org/Defining-columns)), turning on column view in the agenda visits all relevant agenda files and make sure that the computations of this property are up to date. This is also true for the special ‘`CLOCKSUM`’ property. Org then sums the values displayed in the agenda. In the daily/weekly agenda, the sums cover a single day; in all other views they cover the entire block.
