...
Customize `org-odt-convert-process` variable to point to ‘`unoconv`’, which is the ODT’s preferred converter. Working installations of LibreOffice would already have ‘`unoconv`’ installed. Alternatively, other converters may be substituted here. See [Configuring a document converter](/docs/org/Advanced-topics-in-ODT-export).

## Automatically exporting to other formats

If ODT format is just an intermediate step to get to other formats, such as ‘`doc`’, ‘`docx`’, ‘`rtf`’, or ‘`pdf`’, etc., then extend the ODT export backend to directly produce that format. Specify the final format in the `org-odt-preferred-output-format` variable. This is one way to extend (see [ODT export commands](/docs/org/ODT-export-commands)).
