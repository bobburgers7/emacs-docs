...
When `org-id-link-consider-parent-id` is `t`[^2], parent ‘`ID`’ properties are considered. This allows linking to specific targets, named blocks, or headlines (which may not have a globally unique ‘`ID`’ themselves) within the context of a parent headline or file which does.
...
[^1]: The Org Id library must first be loaded, either through `org-customize`, by enabling `id` in `org-modules`, or by adding ‘`(require 'org-id)`’ in your Emacs init file.

[^2]: Also, `org-link-context-for-files` and `org-id-link-use-context` should be both enabled (which they are, by default).
