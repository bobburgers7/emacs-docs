...
When captions follow the method as described in [Captions](/docs/org/Captions), the LaTeX export backend wraps the picture in a floating ‘`figure`’ environment. To float an image without specifying a caption, set the ‘`:float`’ attribute to one of the following:
