...
Note that noweb expansion does not automatically carry over ‘`:var`’ header arguments[^2].
...
Alternatively, Org can include the results of evaluation of a single code block rather than its body \[[^3]. Evaluation occurs when parentheses, possibly including arguments, are appended to the code block name, as shown below.
...
[^1]: For noweb literate programming details, see [https://www.cs.tufts.edu/\~nr/noweb/](https://www.cs.tufts.edu/~nr/noweb/).

[^2]: In the following example, attempting to evaluate the second code block will give an error, because the variables defined in the first code block will not be defined in the second block.

[^3]: The reference is evaluated with point at the referenced block, using its header arguments (including inherited)
