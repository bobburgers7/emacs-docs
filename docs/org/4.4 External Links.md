...
On top of these built-in link types, additional ones are available through the ‘`org-contrib`’ repository (see [Installation](/docs/org/Installation)). For example, these links to VM or Wanderlust messages are available when you load the corresponding libraries from the ‘`org-contrib`’ repository:
