...
You may also link to related files, such as images. Provided you are careful with relative file names, and provided you have also configured Org to upload the related files, these links will work too. See [Example: complex publishing configuration](/docs/org/Complex-example), for an example of this usage.

Links between published documents can contain some search options (see [Search Options in File Links](/docs/org/Search-Options)), which will be resolved to the appropriate location in the linked file. For example, once published to HTML, the following links all point to a dedicated anchor in ‘`foo.html`’.
