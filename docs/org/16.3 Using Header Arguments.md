...
Inline source blocks (see [Structure of Code Blocks](/docs/org/Structure-of-Code-Blocks)) use slightly different default header arguments defined in `org-babel-default-inline-header-args`:
...
Unlike the default values, header arguments set using Org mode properties (see [Header arguments in Org mode properties](/docs/org/Using-Header-Arguments)) do apply to both the normal source blocks and inline source blocks.
