...
Set the priority of the current headline. The command prompts for a priority character ‘`A`’, ‘`B`’ or ‘`C`’. When you press `SPC` instead, the priority cookie, if one is set, is removed from the headline. The priorities can also be changed “remotely" from the agenda buffer with the `,` command (see [Commands in the Agenda Buffer](/docs/org/Agenda-Commands)).

### `S-UP` (`org-priority-up`)

### `S-DOWN` (`org-priority-down`)

Increase/decrease the priority of the current headline[^1]. Note that these keys are also used to modify timestamps (see [Creating Timestamps](/docs/org/Creating-Timestamps)). See also [Packages that conflict with Org mode](/docs/org/Conflicts), for a discussion of the interaction with shift-selection.
