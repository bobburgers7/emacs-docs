...
[During export](/docs/org/Advanced-Export-Configuration), you can include the content of another file. For example, to include your ‘`.emacs`’ file, you could use:
...
Inclusions may specify a file-link to extract an object matched by `org-link-search`[^1] (see [Search Options in File Links](/docs/org/Search-Options)). The ranges for ‘`:lines`’ keyword are relative to the requested element. Therefore,
