...
If an Org headline’s level has no associated Texinfo structuring command, or is below a certain threshold (see [Export Settings](/docs/org/Export-Settings)), then the Texinfo export backend makes it into a list item.
