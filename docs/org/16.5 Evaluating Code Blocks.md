...
A note about security: With code evaluation comes the risk of harm. Org safeguards by prompting for user’s permission before executing any code in the source block. To customize this safeguard, or disable it, see [Code Evaluation and Security Issues](/docs/org/Code-Evaluation-Security).

## How to evaluate source code

Org captures the results of the code block evaluation and inserts them in the Org file, right after the code block. The insertion point is after a newline and the ‘`RESULTS`’ keyword. Org creates the ‘`RESULTS`’ keyword if one is not already there. More details in [Results of Evaluation](/docs/org/Results-of-Evaluation).
...
If ‘`eval`’ header argument is not set, then Org determines whether to evaluate the source code from the `org-confirm-babel-evaluate` variable (see [Code Evaluation and Security Issues](/docs/org/Code-Evaluation-Security)).
...
[^2]: Actually, the constructs ‘`call_<name>()`’ and ‘`src_<lang>{}`’ are not evaluated when they appear in a keyword (see [Summary of In-Buffer Settings](/docs/org/In_002dbuffer-Settings)).
