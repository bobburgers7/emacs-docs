...
Do a regular expression search for `REGEXP` (see [Regular Expressions](/docs/org/Regular-Expressions)). This uses the Emacs command `occur` to list all matches in a separate window. If the target file is in Org mode, `org-occur` is used to create a sparse tree with the matches.
