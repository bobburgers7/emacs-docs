...
These keys jump from one TODO sub-sequence to the next. In the above example, `C-u C-u C-c C-t` or `C-S-RIGHT` would jump from ‘`TODO`’ or ‘`DONE`’ to ‘`REPORT`’, and any of the words in the second row to ‘`CANCELED`’. Note that the `C-S-` key binding conflict with shift-selection (see [Packages that conflict with Org mode](/docs/org/Conflicts)).

### `S-RIGHT`

### `S-LEFT`

`S-LEFT` and `S-RIGHT` walk through *all* keywords from all sub-sequences, so for example `S-RIGHT` would switch from ‘`DONE`’ to ‘`REPORT`’ in the example above. For a discussion of the interaction with shift-selection, see [Packages that conflict with Org mode](/docs/org/Conflicts).
